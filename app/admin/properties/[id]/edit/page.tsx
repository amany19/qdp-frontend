  ...
  const fetchProperty = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Check amenities
        const hasAmenity = (name: string) => data.amenities?.includes(name) || false;
        ...
        setFormData({
          ...
          // Availability
          availableForRent: data.availableFor?.rent || false,
          availableForSale: data.availableFor?.sale || false,
          rentPrice: data.availableFor?.rentPrice?.toString() || '',
          salePrice: data.availableFor?.salePrice?.toString() || '',
          contractDuration: data.availableFor?.contractDuration?.toString() || '12',
          numberOfInstallments: data.availableFor?.numberOfInstallments?.toString() || '12',
          insuranceDeposit: data.availableFor?.insuranceDeposit?.toString() || '',

          // Specifications
          bedrooms: data.specifications?.bedrooms?.toString() || '',
          bathrooms: data.specifications?.bathrooms?.toString() || '',
          livingRooms: data.specifications?.livingRooms?.toString() || '',
          areaSqm: data.specifications?.areaSqm?.toString() || '',
          floorNumber: data.specifications?.floorNumber?.toString() || '',
          totalFloors: data.specifications?.totalFloors?.toString() || '',
          parkingSpaces: data.specifications?.parkingSpaces?.toString() || '',
          furnishingStatus: data.specifications?.furnishingStatus || 'unfurnished',

          // Property Details
          propertyCondition: data.propertyCondition || 'good',
          facade: data.facade || 'north',

          // Location
          address: data.location?.address || '',
          city: data.location?.city || 'Doha',
          area: data.location?.area || '',
          building: data.location?.building || '',
          landmark: data.location?.landmark || '',
          latitude: data.location?.coordinates?.coordinates?.[1]?.toString() || '',
          longitude: data.location?.coordinates?.coordinates?.[0]?.toString() || '',
          ...
          // Amenities
          amenity_security: hasAmenity('24/7 Security'),
          amenity_parking: hasAmenity('Covered Parking'),
          amenity_elevator: hasAmenity('Elevator'),
          amenity_ac: hasAmenity('Central AC'),
          amenity_gym: hasAmenity('Gym'),
          amenity_pool: hasAmenity('Swimming Pool'),
          amenity_garden: hasAmenity('Garden'),
          amenity_balcony: hasAmenity('Balcony'),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        category: formData.availableForRent ? 'rent' : 'sale',
      ...
      if (formData.availableForRent) {
        updateData.availableFor.rentPrice = parseFloat(formData.rentPrice) || 0;
        updateData.availableFor.contractDuration = parseInt(formData.contractDuration) || 12;
        updateData.availableFor.numberOfInstallments = parseInt(formData.numberOfInstallments) || 12;
        updateData.availableFor.insuranceDeposit = parseFloat(formData.insuranceDeposit) || 0;
      }

      // Add sale-specific fields
      if (formData.availableForSale) {
        updateData.availableFor.salePrice = parseFloat(formData.salePrice) || 0;
      }

      // Add optional specification fields
      if (formData.bedrooms) updateData.specifications.bedrooms = parseInt(formData.bedrooms);
      if (formData.bathrooms) updateData.specifications.bathrooms = parseInt(formData.bathrooms);
      if (formData.livingRooms) updateData.specifications.livingRooms = parseInt(formData.livingRooms);
      if (formData.floorNumber) updateData.specifications.floorNumber = parseInt(formData.floorNumber);
      ...
      if (response.ok) {
        ...
        alert(`خطأ: ${error.message || 'فشل في التحديث'}`);
      }
    } catch (error) {
